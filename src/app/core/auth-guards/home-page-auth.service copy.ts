...
export class HomePageAuthService {
    ...
    private doCheckAccessToken(): boolean {
        ...
        setTimeout(() => {
            console.log("HomeAuthService.doCheckAccessToken() -> access_token:",localStorage.getItem(accessTokenKey));
    ...
    public getAccessToken(): void {
        ...
        if (userNeedsNewToken) {
            // Como hago llamadas a la API para obtener los códigos del cliente, hago que la función espere
            setTimeout(async () => {
